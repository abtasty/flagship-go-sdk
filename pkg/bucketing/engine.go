...
func (b *Engine) GetModifications(visitorID string, context model.Context) (*model.APIClientResponse, error) {
	if b.getConfig() == nil {
	...
	for _, c := range config.Campaigns {
		...
		if matchedVg != nil {
			...
			if variation == nil {
				variationCommon, err = commonDecision.GetRandomAllocation(visitorID, matchedVg.ToCommonStruct())
				for _, v := range matchedVg.Variations {
